...
export const disableHardware = async (id: number, reason: string): Promise<HttpStatus> => {
    ...
    if (material.length === 1) {
        ...
        if (email.length === 1) {
            ...
            const text = `<div style="text-align: center;">Votre demande de matériel : <span style="color: #096a09; font-weight: bold;">${material[0].material_description}</span> a été refusée${reasonExist ? ` pour le motif suivant : <br><br> ${reason}` : ""}</div>`;
