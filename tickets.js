    ...
    app.get('/tickets/admin-tickets/', (req, res) => {
        if(!req.session['logged_in']){
            ...
            if(req.session['user_rank'] != "admin")
            {
                ...
                req.session.returnTo = '/tickets/admin-tickets/';
                connection.query('SELECT * FROM tickets WHERE state=1', (errors, closed_tickets, fields) => {
                    connection.query('SELECT * FROM tickets WHERE state=0', (errors, opened_tickets, fields) => {
